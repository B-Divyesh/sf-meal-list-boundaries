...
const BUILD_ID = '1.0.0-r2';
const PRODUCT_ORIGIN = 'https://meal-list-boundaries.sociobot.in';
...
function setPageMetadata(title: string, description: string, path: string): void {
  ...
  document.querySelector<HTMLMetaElement>('meta[property="og:url"]')?.setAttribute('content', `${PRODUCT_ORIGIN}${path}`);
...
function shell(content: string, shared = false): string {
  return `
    <header class="site-header">
      ...
      <nav class="site-nav" aria-label="Main navigation"><a href="/#planner">Planner</a><a href="/demo">Demo</a><a href="/privacy/">Privacy</a></nav>
      <div class="header-actions">
        <span class="connection" id="connection-state" role="status"><i></i>${navigator.onLine ? 'Saved on this device' : 'Offline · changes stay on this device'}</span>
    ...
    ${demoMode ? `<aside class="demo-banner" aria-label="Demo controls"><span><b>Demo</b> — sample data, nothing is saved to your planner.</span><span><button type="button" data-action="reset-demo">Reset demo</button><button type="button" data-action="start-real">Start for real</button></span></aside>` : ''}
    ${content}
    <footer>
      <p>Make separate shopping lists from one weekly meal plan.</p>
      <nav aria-label="Footer navigation"><a href="/privacy/">Privacy</a><a href="/terms/">Terms</a><a href="https://github.com/B-Divyesh/sf-meal-list-boundaries" rel="noreferrer">Source</a></nav>
      <p class="provenance">Built by Param Factory · Version ${BUILD_ID} · The botanical illustration was generated for this product.</p>
    </footer>
    ...
    <main id="main" tabindex="-1">
      <section class="hero" aria-labelledby="page-title">
        <div class="hero-copy">
          ...
          <h1 id="page-title">Make separate shopping lists for every home</h1>
          <p>For households planning across homes or pickups, each meal sends ingredients to the right shopping list.</p>
          <div class="hero-actions">${demoMode ? '' : `<button class="primary" type="button" data-action="start-demo">Try it with sample data</button><span>Loads four sample meals and two shopping lists.</span>`}</div>
          <ul class="hero-facts" aria-label="Product facts"><li>Meal plans stay on this device.</li><li>Works offline after your first visit.</li><li>Free for two places. Field Kit costs ${PRICE} once.</li></ul>
          <div class="hero-legend" aria-label="How it works"><span><b>1</b> Name each place</span><span><b>2</b> Assign each meal</span><span><b>3</b> Share one list</span></div>
        </div>
        <picture class="hero-plate">
          ...
          <img src="/assets/boundary-field-guide-1024.jpg" width="1024" height="683" alt="Two paper grocery envelopes separated by a fern to show two shopping lists" fetchpriority="high" decoding="async" />
        </picture>
      </section>
      ...
      <section class="workspace" id="planner" aria-labelledby="week-heading">
        <div class="weekbar">
          <div><p class="section-label">Week plan</p><h2 id="week-heading">${weekLabel(currentWeek)}</h2></div>
        ...
        <nav class="view-tabs" aria-label="Planner views">
          ...
          <button type="button" data-view="lists" aria-pressed="${view === 'lists'}"><span>Lists</span><small>${buildLists(state, currentWeek).length} shopping list${buildLists(state, currentWeek).length === 1 ? '' : 's'}</small></button>
        </nav>
        <div class="view-panel">${view === 'plan' ? renderPlan() : renderLists()}</div>
      </section>
      <section class="info-section" aria-labelledby="how-title"><p class="section-label">How it works</p><h2 id="how-title">Turn one meal plan into separate lists</h2><ol><li><b>Name each place.</b> Add a home, pickup, or pantry.</li><li><b>Assign each meal.</b> Every ingredient goes to that place’s list.</li><li><b>Use one list.</b> Copy, share, print, or scan only the list you need.</li></ol></section>
      <section class="info-section" aria-labelledby="privacy-title"><p class="section-label">Privacy and limits</p><h2 id="privacy-title">Your meal plan stays on this device</h2><p>There are no accounts, retailer links, recipe imports, price comparisons, or delivery orders.</p><p>Shared links contain one selected shopping list. Anyone with that link can read it.</p></section>
      <section class="info-section field-kit-public" aria-labelledby="field-kit-title"><p class="section-label">Optional purchase</p><h2 id="field-kit-title">Field Kit costs ${PRICE} once</h2><p>The free planner supports two places. Field Kit adds unlimited places and reusable week templates.</p><a class="primary button-link" href="${BUY_URL}">Buy Field Kit</a><p class="muted">Sociobot hosts checkout. Dodo is the merchant of record. There is no subscription.</p></section>
    </main>
    ${renderMealDialog()}
...
function renderPlan(): string {
  if (state.boundaries.length === 0) {
    return `<section class="empty-state"><span class="pressed-leaf" aria-hidden="true">⌁</span><p class="section-label">Start this week</p><h3>Name the places you shop for</h3><p>A place can be a home, cabin, parent’s pantry, or pickup stop. Each place gets its own shopping list.</p><button class="primary" type="button" data-action="settings">${icon('plus')} Add your first place</button></section>`;
  }
...
function renderMeal(meal: Meal): string {
  ...
  return `<li class="meal ${boundaryClass(boundary?.color ?? COLORS[0])}" data-meal-id="${meal.id}">
    <span class="boundary-tag">${escapeHtml(boundary?.symbol ?? '?')} ${escapeHtml(boundary?.name ?? 'Missing place')}</span>
...
function renderLists(): string {
  ...
  if (!lists.length) {
    return `<section class="empty-state"><span class="pressed-leaf" aria-hidden="true">⌁</span><p class="section-label">No shopping lists yet</p><h3>Your separate lists will appear here</h3><p>Add a meal and choose its place. Every ingredient then goes to that place’s shopping list.</p><button class="primary" type="button" data-action="switch-plan">Plan a meal</button></section>`;
  }
  return `<div class="lists-intro"><div><p class="section-label">Shopping lists</p><h3>${lists.length} separate list${lists.length === 1 ? '' : 's'}</h3></div><button type="button" data-action="export-all">${icon('download')} Export all data</button></div>
...
function renderBoundaryList(list: BoundaryList): string {
  ...
  return `<article class="list-sheet ${boundaryClass(list.boundary.color)}" data-boundary-sheet="${list.boundary.id}">
    <header><div class="list-symbol">${escapeHtml(list.boundary.symbol)}</div><div><p class="section-label">Shopping list</p><h3>${escapeHtml(list.boundary.name)}</h3><p>${bought} of ${list.items.length} bought</p></div></header>
...
function renderMealDialog(): string {
  return `<dialog id="meal-dialog" aria-labelledby="meal-dialog-title"><form method="dialog" id="meal-form">
    <div class="dialog-heading"><div><p class="section-label">Meal details</p><h2 id="meal-dialog-title">Add a meal</h2></div><button class="icon-button" type="button" data-action="close-dialog" aria-label="Close meal form">×</button></div>
    ...
    <label>Shopping list<select name="boundaryId" required>${state.boundaries.map((boundary) => `<option value="${boundary.id}">${escapeHtml(boundary.symbol)} ${escapeHtml(boundary.name)}</option>`).join('')}</select><small>Every ingredient below goes to this shopping list.</small></label>
...
function renderSettingsDialog(): string {
  return `<dialog id="settings-dialog" class="wide-dialog" aria-labelledby="settings-title"><div class="dialog-heading"><div><p class="section-label">Planner settings</p><h2 id="settings-title">Places & data</h2></div><button class="icon-button" type="button" data-action="close-dialog" aria-label="Close settings">×</button></div>
    <section><h3>Your places</h3><p class="muted">The free planner includes two places. Field Kit adds as many as your week needs.</p>
      <ul class="boundary-settings">${state.boundaries.map((boundary) => `<li class="${boundaryClass(boundary.color)}"><span>${escapeHtml(boundary.symbol)}</span><form data-form="rename-boundary"><input type="hidden" name="id" value="${boundary.id}"/><label><span class="sr-only">Place name</span><input name="name" maxlength="40" required value="${escapeHtml(boundary.name)}"/></label><button type="submit">Save</button><button class="danger-text" type="button" data-action="delete-boundary" data-id="${boundary.id}">Remove</button></form></li>`).join('')}</ul>
      <form class="inline-form" data-form="add-boundary"><label><span>New place name</span><input name="name" maxlength="40" required placeholder="e.g. Lake house"/></label><button class="primary" type="submit">${icon('plus')} Add place</button></form>
    </section>
...
function renderFieldKit(): string {
  if (licensed) {
    return `<section class="settings-section field-kit"><p class="section-label">Field Kit active</p><h3>Reusable week templates</h3><p class="muted">Save this week’s meals, then add them to another week. Each meal keeps its shopping list.</p>
  ...
  return `<section class="settings-section field-kit"><p class="section-label">Optional one-time purchase</p><h3>Field Kit · ${PRICE} once</h3><p>The free planner supports two places. Field Kit adds unlimited places and reusable week templates. No subscription.</p><div class="button-row"><a class="primary button-link" href="${BUY_URL}">Buy Field Kit</a></div><form class="license-form" data-form="restore-license"><label><span>Have a license? Paste it here</span><input name="license" required autocomplete="off"/></label><button type="submit">Verify license</button></form><p class="muted">Sociobot hosts checkout. Dodo is the merchant of record. Refunds revoke the license. <a href="/privacy/">Privacy</a> · <a href="/terms/">Terms</a></p></section>`;
}

function renderQrDialog(): string {
  return `<dialog id="qr-dialog" aria-labelledby="qr-title"><div class="dialog-heading"><div><p class="section-label">Share one shopping list</p><h2 id="qr-title">Scan to open this list</h2></div><button class="icon-button" type="button" data-action="close-dialog" aria-label="Close QR code">×</button></div><p id="qr-description">The link contains only this list. No account or server copy is created.</p><canvas id="qr-canvas" width="280" height="280" aria-label="QR code for this shopping list"></canvas><div class="dialog-actions"><button type="button" id="copy-qr-link" data-url="">Copy link</button><button class="primary" type="button" data-action="close-dialog">Done</button></div></dialog>`;
}

function renderShared(encoded: string): void {
  try {
    const payload = decodePayload(encoded);
    setPageMetadata('Shared list — Meal List Boundaries', `Open the ${payload.boundary.name} shopping list on this device.`, '/');
    ...
    root.innerHTML = shell(`<main id="main" tabindex="-1" class="shared-main"><section class="shared-intro"><p class="eyebrow">Shared shopping list · ${escapeHtml(weekLabel(payload.weekStart))}</p><h1>${escapeHtml(payload.boundary.symbol)} ${escapeHtml(payload.boundary.name)} shopping list</h1><p>This link contains one shopping list. Mark items as bought; progress stays on this device.</p></section><article class="list-sheet shared-sheet ${boundaryClass(payload.boundary.color)}"><header><div class="list-symbol">${escapeHtml(payload.boundary.symbol)}</div><div><p class="section-label">Shopping list</p><h2>${escapeHtml(payload.boundary.name)}</h2><p>${payload.items.length} item${payload.items.length === 1 ? '' : 's'}</p></div></header><ul class="shopping-list">${payload.items.map((item, index) => `<li class="${checked.has(String(index)) ? 'bought' : ''}"><label><input type="checkbox" data-shared-index="${index}" ${checked.has(String(index)) ? 'checked' : ''}/><span><b>${escapeHtml(item.text)}</b>${item.count > 1 ? `<small> × ${item.count}</small>` : ''}</span></label></li>`).join('')}</ul><div class="list-actions"><button type="button" data-action="print-shared">Print</button><a href="/">Open my planner</a></div></article></main>`, true);
    ...
    setPageMetadata('Invalid shared list — Meal List Boundaries', 'This shared shopping list could not be opened.', '/');
    const message = error instanceof Error ? error.message : 'This shared link is invalid.';
    root.innerHTML = shell(`<main id="main" tabindex="-1" class="shared-main"><section class="empty-state"><p class="section-label">Shared list unavailable</p><h1>This list could not be opened</h1><p>${escapeHtml(message)}</p><a class="button-link primary" href="/">Open my planner</a></section></main>`, true);
  }
}
...
root.addEventListener('click', async (event) => {
  ...
  if (action === 'theme') {
    ...
    await persist(); render(); showToast(state.bought[key] ? 'Marked as bought.' : 'Returned to the list.', async () => { state.bought[key] = previous; await persist(); render(); });
    ...
    if (boundary && confirm(`Remove “${boundary.name}”${meals ? ` and its ${meals} meal${meals === 1 ? '' : 's'}` : ''}? This cannot be undone.`)) { state.boundaries = state.boundaries.filter((item) => item.id !== boundary.id); state.meals = state.meals.filter((item) => item.boundaryId !== boundary.id); await persist('Place removed.'); render(); reopenSettings(); }
...
root.addEventListener('submit', async (event) => {
  ...
  if (form.id === 'meal-form') {
    ...
    form.closest<HTMLDialogElement>('dialog')?.close(); render(); await persist(existing ? 'Meal updated.' : 'Meal added to its shopping list.');
  } else if (form.dataset.form === 'add-boundary') {
    if (!licensed && state.boundaries.length >= 2) { showToast('The free planner includes two places. Field Kit adds more.'); return; }
    const name = String(data.get('name') ?? '').trim(); if (!name) return; const index = state.boundaries.length;
    state.boundaries.push({ id: crypto.randomUUID(), name, symbol: SYMBOLS[index % SYMBOLS.length], color: COLORS[index % COLORS.length] }); render(); reopenSettings(); await persist('Place added.');
  } else if (form.dataset.form === 'rename-boundary') {
    const boundary = state.boundaries.find((item) => item.id === data.get('id')); const name = String(data.get('name') ?? '').trim(); if (boundary && name) boundary.name = name; await persist('Place name saved.'); render(); reopenSettings();
  } else if (form.dataset.form === 'restore-license') {
    const token = String(data.get('license') ?? '').trim(); saveLicense(token); showToast('Checking your license…'); const result = await verifyLicense(true); licensed = result.valid; licenseNotice = result.valid ? '' : result.offline ? 'Could not reach license verification. Your free planner remains available.' : 'That license is not active for this product.'; render(); if (licensed) { showToast('Field Kit is active on this device.'); reopenSettings(); }
    ...
    registration.addEventListener('updatefound', () => {
      ...
      worker?.addEventListener('statechange', () => {
        if (worker.state === 'installed' && navigator.serviceWorker.controller) {
          showToast('An app update is ready.', () => { worker.postMessage({ type: 'SKIP_WAITING' }); location.reload(); }, 'Reload');
        }
      });
    });
...
async function start(): Promise<void> {
  applyTheme();
  if (demoMode) setPageMetadata('Demo — Meal List Boundaries', 'Try separate Home and Cabin shopping lists with sample meals.', '/demo');
  else setPageMetadata('Meal List Boundaries — Separate shopping lists', 'Make separate shopping lists for each home or pickup from one weekly meal plan.', '/');
